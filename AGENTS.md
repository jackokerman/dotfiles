...
- Keep `HOMEBREW_DOTFILES_ENV=remote` installs serialized in `scripts/brew-sync.sh`; concurrent Linuxbrew downloads and formula jobs have caused intermittent bootstrap failures through shared prefix state. The remote bundle may retry once as a complete idempotent operation, but the retry must remain blocking and propagate a second failure.
