...
- Use `dotty update` when catching a machine up to the repo state. Use `dotty run brew-sync` when you want to install tracked Homebrew packages on supported hosts, and use `dotty run brew-sync --cleanup` only when you explicitly want to remove untracked Homebrew packages. Use `dotty run macos-setup` when you want to reapply tracked macOS defaults and related setup.
- Keep personal-only Homebrew entries conditional inside `Brewfile` on `HOMEBREW_DOTFILES_ENV=personal`; shell startup sets that Homebrew-preserved variable because Homebrew strips ordinary dotty env vars before evaluating Brewfiles. Tools that are supplied by host-specific managed setup belong in local overrides or later repos, not in the base Brewfile.
- Keep remote Linux formula installation serialized in `scripts/brew-sync.sh`. Install the tracked `agavra/tap/tuicr` formula alone with serialized downloads and at most one targeted retry before allowing concurrent downloads for the unchanged Bundle. The remote Bundle may retry once as a complete idempotent operation, but both targeted and Bundle retries must remain blocking and propagate a second failure.
...
- After changing tracked Karabiner or macOS-setup sources, do not assume `dotty update` reapplies them. Run `bun --install=fallback run scripts/ts/karabiner-config.ts` for a narrow keyboard-remap refresh or `dotty run macos-setup` for the broader macOS setup path.
...
- Preserve the `prefix` + `F` Fleet popup and `prefix` + `f` sidebar bindings. Do not add Fleet's optional `prefix` + `n` or window-state rollup without a separate workflow decision.
...
- User-installed completions live under `~/.local/share/zsh/site-functions`, and Homebrew completions under the active Homebrew prefix's `share/zsh/site-functions` directory.
