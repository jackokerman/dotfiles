...
- Managed runtime checkouts live under `~/.local/share/` when the checkout is an implementation detail rather than a contribution workspace. Actively developed tools such as `tmux-agent-bar` use `~/src` through `.dotty/devtools.tsv`.
- Development repos for reusable personal tools live under `~/src` and are listed in `.dotty/devtools.tsv`; `dotty update` clones missing entries, conservatively fast-forwards clean checkouts, and runs configured install actions.
...
- Jackie Plan is an exception to the runtime-checkout pattern: `.dotty/devtools.tsv` installs it from the normal development checkout at `~/src/jackie-plan`, and the Jackie Plan repo installer owns its `jp` CLI and Codex plugin installation.
