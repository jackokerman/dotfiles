...
- Keep the generic sesh picker and one-shot launcher helpers in `home/.local/bin/`. Keep portable sesh defaults in `home/.config/sesh/sesh.toml`; `~/.config/sesh/sesh.toml` is generated from tracked fragments during `dotty update`, and later repos in the dotty chain own their own session definitions.
...
- The optional GSD Core checkout is managed under `~/.local/share/gsd-core/repo` by `dotty run install-gsd-core`; keep it separate from upstream contribution clones and use the command's uninstall mode to disable automatic reapply.
