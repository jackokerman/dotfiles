...
- Keep Linux formula installation serialized in `scripts/brew-sync.sh`. Install the tracked `agavra/tap/tuicr` formula alone with serialized downloads and at most one targeted retry before allowing concurrent downloads for the unchanged Bundle. The Linux Bundle may retry once as a complete idempotent operation, but both targeted and Bundle retries must remain blocking and propagate a second failure.
