...
- Codex uses the same pattern: `home/.codex/AGENTS.md`, `home/.codex/config.toml`, and `home/.codex/hooks.json` are tracked source fragments, while the live `~/.codex` files are generated by the hook so local runtime state is preserved. The managed Codex config also sets `approval_policy = "never"`, `sandbox_mode = "danger-full-access"`, `commit_attribution = ""`, and enables `features.codex_hooks = true`.
