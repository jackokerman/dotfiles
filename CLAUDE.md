...
- Codex uses the same pattern: `home/.codex/AGENTS.md` and `home/.codex/config.toml` are tracked source fragments, while the live `~/.codex` files are generated by the hook so local runtime state is preserved.
