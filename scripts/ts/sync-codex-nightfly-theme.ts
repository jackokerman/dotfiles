...
const vimNightflySubmoduleRepoPath =
	'home/.codex/references/nightfly/vim-nightfly-colors';
const fly16SubmodulePath = resolve(projectRoot, fly16SubmoduleRepoPath);
const vimNightflySubmodulePath = resolve(
	projectRoot,
	vimNightflySubmoduleRepoPath,
);
const fly16SourcePath = resolve(fly16SubmodulePath, 'fly16.tmTheme');
const opencodeSourcePath = resolve(
	vimNightflySubmodulePath,
	'extras/nightfly-opencode.json',
);
const nightflyVimPalettePath = resolve(
	vimNightflySubmodulePath,
	'autoload/nightfly.vim',
);
...
function usage() {
	return [
		'Usage: bun scripts/ts/sync-codex-nightfly-theme.ts [--check] [--refresh-upstream]',
		'',
	...
	if (!existsSync(path)) {
		fail(
			[
				...
				'- `bun scripts/ts/sync-codex-nightfly-theme.ts --refresh-upstream`',
			].join('\n'),
		);
	}
...
function generateTheme() {
	...
	getRuleSettings(findRuleByScope(rules, 'meta.separator')).background =
		nightflyPalette.regalBlue;
	getRuleSettings(findRuleByScope(rules, 'markup.inserted')).background =
		resolveThemeColor(defs, theme, 'diffAddedBg');
	getRuleSettings(findRuleByScope(rules, 'markup.deleted')).background =
		resolveThemeColor(defs, theme, 'diffRemovedBg');

	const changedSettings = getRuleSettings(
		findRuleByScope(rules, 'markup.changed'),
	);
	...
	const headerComment = [
		'<!--',
		'Generated by `scripts/ts/sync-codex-nightfly-theme.ts`.',
		'',
		...
		'- `bun scripts/ts/sync-codex-nightfly-theme.ts`',
		'- `bun scripts/ts/sync-codex-nightfly-theme.ts --refresh-upstream`',
		'-->',
	].join('\n');
...
async function main() {
	...
	if (args.has('--check')) {
		...
		if (currentTheme !== generatedTheme) {
			fail(
				`Tracked Nightfly theme is out of date. Run \`bun scripts/ts/sync-codex-nightfly-theme.ts\` to regenerate it.`,
			);
		}
		return;
	}
