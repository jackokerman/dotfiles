...
function parseSkillFrontmatter(text: string, context: string) {
	...
	if (description.length > 1024) {
		throw new Error(
			`Skill description is too long in ${context}: ${description.length}`,
		);
	}
...
function collectSkillSources(sourceRoots: string[]) {
	...
	for (const root of roots) {
		for (const sourceDir of listDirectories(root)) {
			...
			const {description, name} = parseSkillFrontmatter(
...
function parseManagedIndex(path: string) {
	...
	return rowLines.filter(Boolean).map((line) => {
...
function writeManagedIndex(
	path: string,
	headers: string[],
	rows: ManagedIndexRow[],
) {
	const body = [
		headers.join('\t'),
...
function replaceDirectory(targetDir: string, sourceDir: string) {
	const stagingRoot = mkdtempSync(
		join(dirname(targetDir), '.tmp-codex-skill-'),
	);
...
function resolveSkillReference(
	...
	if (pathValue.startsWith('skill://')) {
		...
		if (!skillSources.has(skillName)) {
			throw new Error(
				`Unknown tracked skill "${skillName}" referenced by ${sourceFile}`,
			);
		}
...
function normalizeAgentDocument(
	...
	if (description.length > 1024) {
		throw new Error(
			`Agent description is too long in ${sourceFile}: ${description.length}`,
		);
	}
	...
	if (
		...
		throw new Error(
			`Expected non-empty developer_instructions in ${sourceFile}`,
		);
	}
	...
	const skillsTable = structuredClone(doc.skills as Record<string, TomlValue>);
	skillsTable.config = skillEntries.map((entry, index) => {
		if (!isObject(entry)) {
			throw new Error(
				`Expected skill config table at ${sourceFile} entry ${index + 1}`,
			);
		}
		...
		if (typeof pathValue !== 'string' || pathValue.length === 0) {
			throw new Error(
				`Expected skill config path at ${sourceFile} entry ${index + 1}`,
			);
		}

		return {
			...structuredClone(entry as Record<string, TomlValue>),
			path: resolveSkillReference(
				pathValue,
				sourceFile,
				skillSources,
				skillsOutput,
	...
	for (const root of roots) {
		for (const sourceFile of listTomlFiles(root)) {
			...
			const body = stringify(
				normalizeAgentDocument(sourceFile, name, skills, skillsOutput),
			);
...
function syncCustomAgents(
	...
	const {agents} = collectAgentSources(
		sourceRoots,
		skillSourceRoots,
		skillsOutput,
	);
	...
	writeManagedIndex(
		indexPath,
		['agent_name', 'source_file', 'target_file'],
		rows,
	);
}
...
function renderConfig(
	...
	let merged = validateOnly ? {} : structuredClone(loadToml(outputPath));
...
function validateHookAction(entry: JsonValue, context: string): HookAction {
	...
	return structuredClone(entry as HookAction);
}

function validateHookEventEntry(
	entry: JsonValue,
	context: string,
): HookEventEntry {
	if (!isObject(entry)) {
	...
	return {
		...structuredClone(entry as JsonObject),
		hooks,
	};
}
...
function loadHooks(path: string) {
	...
	for (const [eventName, eventHooks] of Object.entries(rawHooks)) {
		if (!Array.isArray(eventHooks)) {
			throw new Error(
				`Expected hook array for event "${eventName}" in ${path}`,
			);
		}
	...
	for (const source of sources) {
		...
		for (const [eventName, eventHooks] of Object.entries(parsed)) {
			...
			hooks[eventName].push(
				...eventHooks.map((entry) => structuredClone(entry)),
			);
		}
	}
...
function main() {
	const {mode, output, skillSources, skillsOutput, sources, validateOnly} =
