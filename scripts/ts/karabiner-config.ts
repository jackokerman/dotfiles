...
const nonTouchIdMagicKeyboardCondition = ifDevice(
	appleMagicKeyboardWithTouchId,
).unless();
...
function isTouchIdMagicKeyboard(device: KarabinerDevice) {
	return (
		device.identifiers?.product_id ===
			appleMagicKeyboardWithTouchId.product_id &&
...
async function normalizeKeyboardDevices(configPath: string) {
	...
	const profile = config.profiles?.find(
		(candidate) => candidate.name === PROFILE_NAME,
	);
	...
	writeToProfile(PROFILE_NAME, [
		rule(
			...
			withCondition(builtInKeyboardCondition)([
				map('caps_lock').to('left_control').toIfAlone('caps_lock'),
			]),
		]),
		rule(
