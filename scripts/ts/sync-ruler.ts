...
function replaceDirectory(targetDir: string, sourceDir: string) {
	const stagingRoot = mkdtempSync(
		join(dirname(targetDir), '.tmp-ruler-skill-'),
	);
...
function installGeneratedSkills(
	...
	for (const row of previousRows) {
		...
		if (
			sourceDir &&
			(sourceLooksPortable(sourceDir) || !sourceStillLooksLikeSkill(sourceDir))
		) {
			rmSync(targetDir, {force: true, recursive: true});
			continue;
		}
	...
	for (const skill of stagedSkills) {
		...
		if (!existsSync(resolve(generatedDir, 'SKILL.md'))) {
			throw new Error(
				`Ruler did not generate expected skill ${skill.name} at ${generatedDir}`,
			);
		}
...
function syncCodexAgents(
	...
	try {
		...
		writeManagedFile(outputPath, generatedBody, [
...
function syncPortable(
	...
	const {sources, stagedSkills, stagingRoot} = stageRulerProject(
		sourcePaths,
		skillSourcePaths,
		agents,
	);
	...
	try {
		...
		writeManagedFile(
			codexAgentsOutput,
			readFileSync(generatedCodexAgents, 'utf8'),
			[
				'Generated by dotty from tracked Ruler instruction sources.',
				'Edit one of these source files, then run `dotty update`.',
				...sources.map((source) => `Source: ${source}`),
		...
		installGeneratedSkills(
			[
				join(stagingRoot, '.agents', 'skills'),
				join(stagingRoot, '.codex', 'skills'),
			],
			codexSkillsOutput,
			stagedSkills,
			'Codex',
