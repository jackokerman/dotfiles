...
function parseStringArray(value: unknown, field: string, sourceFile: string) {
	...
	const items = value.map((entry, index) => {
		if (typeof entry !== 'string' || entry.length === 0) {
			throw new Error(
				`Expected non-empty string at ${field}[${index}] in ${sourceFile}`,
			);
		}
...
function parseManifest(manifestSources: string[]) {
	...
	const defaultSkills = parseStringArray(
		doc.default_skills,
		'default_skills',
		sourceFile,
	);
	const optionalSkills = parseStringArray(
		doc.optional_skills,
		'optional_skills',
		sourceFile,
	);
...
function validateSkillMetadata(skill: SkillSource) {
	if (!existsSync(skill.skillFile)) {
		throw new Error(
			`Missing SKILL.md for tracked skill "${skill.name}" at ${skill.skillFile}`,
		);
	}
	...
	const defaultPrompt = assertStringField(
		interfaceConfig,
		'default_prompt',
		skill.openaiYamlFile,
	);
...
function extractSkillReferenceName(
	pathValue: string,
	sourceFile: string,
	index: number,
) {
	if (pathValue.startsWith('skill://')) {
		...
		if (name.length === 0) {
			throw new Error(
				`Expected skill name in ${sourceFile} entry ${index + 1}`,
			);
		}
...
function extractAttachedSkills(agent: AgentSource) {
	...
	const entries = Array.isArray(configEntries)
		? configEntries
		: [configEntries];
	return entries.map((entry, index) => {
		if (!isObject(entry)) {
			throw new Error(
				`Expected [skills.config] object in ${agent.sourceFile} entry ${index + 1}`,
			);
		}
		...
		if (typeof pathValue !== 'string' || pathValue.length === 0) {
			throw new Error(
				`Expected skill path in ${agent.sourceFile} entry ${index + 1}`,
			);
		}
...
function validateManifest(
	...
	if (manifest.excludedTaskCategories.length === 0) {
		throw new Error(
			`Expected excluded_task_categories entries in ${manifest.sourceFile}`,
		);
	}
	...
	if (
		...
		attachedSkills.some(
			(skillName, index) => skillName !== manifest.defaultSkills[index],
		)
	) {
		throw new Error(
...
function main() {
	const {manifestSources, agentSources, skillSources} = parseArgs(
