...
- `home/.config/tmux/tmux.conf` binds `prefix` + `F` to the 55% by 60% dashboard popup, `prefix` + `f` to the native 34-column sidebar, and `prefix` + `n` to the next waiting agent in Fleet's priority order. It injects Fleet's attention-only second status row after Nightfly loads. The row always shows its sidebar button and adds agents only when they need permission, have a question, or are ready for review.
