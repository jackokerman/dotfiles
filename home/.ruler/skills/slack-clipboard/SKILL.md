...
- For manual composer paste, keep mentions as `@handle` text. Do not convert them to `<@USER_ID>` syntax for clipboard payloads; that syntax can paste or post literally, while Slack can resolve pasted handles when the message is sent. Use authenticated Slack delivery tools when deterministic mentions are required.
