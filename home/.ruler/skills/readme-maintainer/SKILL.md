...
4. Edit for scanability.
   ...
   - When a fenced command quick-reference block uses trailing `#` descriptions, align the `#` comments only if the block stays narrow enough to scan in a normal GitHub viewport; otherwise split the block by workflow or move explanations into prose before each shorter block.
