...
- For commit-like validation in dotfiles repos, use `./scripts/check --staged --quiet` and rely on the pre-commit hook result instead of manually rerunning the same staged path after commit. Use full `./scripts/check --quiet` only for broad workflow, hook, generated-config, tmux, remote-session, or check-runner changes.
