...
- Before finishing in repos with a repo-local `.jackie-plan/`, re-check `git status --short` for newly created plan artifacts. Commit intentional repo-local backlog items in that repo, and move misrouted plans to the correct plan root instead of leaving them unstaged.
