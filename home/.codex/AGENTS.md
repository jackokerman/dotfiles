...
- When 1Password CLI access is needed, run `op signin` directly and let the user approve Touch ID. Pass secret values through JSON or standard input, never command arguments or output.
...
- For generated skills, use the managed index to locate the tracked source under `home/.codex/skills/`.

## Workflow Skills

- Use `writing-style` before substantial drafting and `readme-maintainer` for README or repository landing-page work.
