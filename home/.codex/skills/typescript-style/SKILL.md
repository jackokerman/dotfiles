...
- For non-temporary JS/TS tools, prefer Bun, TypeScript, Oxlint, and repo-local deterministic quality gates unless the surrounding project has a stronger established stack.
...
- Put reusable lint style changes in the repository that owns the shared config first. Update dependents only when exported lint behavior changes; do not churn dependents for docs-only, README-only, or source-repo check-tooling changes.
...
- Write JSDoc prose as complete sentences with explicit subjects. Avoid clipped follow-on fragments when a phrase like `this queues one more pass` is clearer.
