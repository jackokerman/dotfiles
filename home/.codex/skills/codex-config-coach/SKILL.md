...
7. At closeout after a skill or agent-config edit, or after any heavy/correction-heavy workflow, do a lightweight token/determinism pass without waiting for the user to ask. Fold clear scope-local fixes into the same review packet; capture separate follow-ups only when they would distract from the current change.
