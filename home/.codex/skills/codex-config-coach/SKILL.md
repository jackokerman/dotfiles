...
2. Before editing, search repo `AGENTS.md`, tracked `home/.ruler/`, `home/.codex/`, `home/.claude/`, and nearby skills for overlap. Choose one authoritative owner and replace or consolidate before appending; inspect live generated files only to confirm runtime state. For generated skills, use `.dotty-managed-skills.tsv` or the owner manifest to find the tracked source.
3. Audit cost and determinism early; record the before/after active or deferred size for each changed context surface and justify the delta. Treat always-active instructions, long skill text, enabled plugins, active MCPs, runtime-injected tools, duplicate reads, broad searches, manual multi-source gathering, and polling as suspect until they prove useful. Prefer CLIs, repo helpers, deferred tool discovery, or skill-triggered surfaces over always-on MCP/plugin context when the workflow is occasional.
4. Measure when practical. For skills or plugins, run `plugin-eval analyze <path> --format markdown`; add `plugin-eval explain-budget <path> --format markdown` when token cost is part of the concern, and validate skill structure with `validate-codex-skill <path>`. For MCP changes, capture enabled server count, exposed tool count, and approximate `tools/list` schema bytes before and after when the server can report them cheaply. Use scores and counts as evidence, not as the only objective.
...
7. At closeout after a skill or agent-config edit, or after any heavy/correction-heavy workflow, do a lightweight token/determinism pass without waiting for the user to ask. Implement clear, bounded skill or config fixes immediately, even when they are adjacent to the main task. Capture a follow-up only when the change is materially larger, riskier, or blocked; do not create backlog for a small edit that can be completed and verified in the current session.
